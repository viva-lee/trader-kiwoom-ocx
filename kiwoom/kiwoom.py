...
class Kiwoom(QAxWidget):
    ...
    def real_event_slot(self):
        ...
        self.OnReceiveMsg.connect(self.msg_slot) # 서버 메세지 이벤트
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        if sRQName == "예수금상세현황요청":
            ...
            for i in range(rows):
                code = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "종목코드") # 100개까지 카운트
            ...
            if sPrevNext == "2":
                ...
                if self.calcul_data == None or len(self.calcul_data) < 120:
                    ...
                    if int(self.calcul_data[0][7]) <= moving_average_price and moving_average_price <= int(self.calcul_data[0][6]):
                        print("오늘의 주가가 120 이평선에 근처에 있는지 확인")
                    ...
                    if bottom_stock_price == True:
                        ...
                        while True:
                            ...
                            if moving_average_price_prev <= int(self.calcul_data[idx][6]) and idx <= 20:
                                print("20일 동안 주가가 120일 이평선과 같거나 위에 있으면 조건 통과 x")
                                price_top_moving = False
                                break
    ...
    def read_code(self):
        if os.path.exists("files/condition_stock.txt"):
            ...
            lines = f.readlines() # 파일에 있는 내용 읽기
            for line in lines:
                if line != "":
                    ls = line.split("\t")
    ...
    def screen_number_setting(self):
        ...
        for code in screen_overwrite:
            ...
            if code in self.portfolio_stock_dict.keys():
                ...
                self.portfolio_stock_dict[code].update({"주문용스크린번호": str(self.screen_real_stock)})
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFidList):
        if int(sGubun) == 0: # 주문체결
            ...
            meme_gubun = self.realType.REALTYPE["매도수고분"][meme_gubun]
    ...
    def msg_slot(self, sCcrNo, sRQName, sTrCode, msg):
        print("스크린: %s, 요청이름: %s, tr코드: %s --- %s" % (sCcrNo,sRQName, sTrCode, msg))
